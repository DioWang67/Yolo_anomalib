...
class _FakeInnerModel:
    def __init__(self): self.half_called = False
    def half(self): self.half_called = True
...
class FakeDetector:
    ...
    def process_detections(self, pred, processed_image, image, expected_items):
        ...
        detections = [{"bbox": [10, 20, 50, 60], "label": (expected_items[0] if expected_items else "unk")}]
        missing = set()
...
def model_cpu(cfg_cpu, monkeypatch):
    ...
    # letterbox 不改變像素（保流程）
    monkeypatch.setattr(m.image_utils, "letterbox", lambda img, size, fill_color: img, raising=True)
    return m
...
def test_cache_eviction_lru(monkeypatch):
    ...
    monkeypatch.setattr(yim, "YOLO", FakeYOLO, raising=True)
    monkeypatch.setattr(yim, "YOLODetector", FakeDetector, raising=True)
    m = YOLOInferenceModel(cfg); m.logger = DummyLogger()
...
def test_preprocess_image_calls_letterbox(model_cpu, image_bgr_dark, monkeypatch):
    ...
    def fake_letterbox(img, size, fill_color):
        called["letterbox"] += 1
        return img
    monkeypatch.setattr(model_cpu.image_utils, "letterbox", fake_letterbox, raising=True)
...
def test_infer_happy_path(model_cpu, cfg_cpu, image_bgr_dark, monkeypatch):
    ...
    # 檢查輸出契約
    required = {"inference_type","status","detections","missing_items","inference_time",
                "processed_image","result_frame","expected_items"}
    ...
    assert "cx" in det and "cy" in det and "image_width" in det and "image_height" in det
    assert det["cx"] == (10 + 50) / 2 and det["cy"] == (20 + 60) / 2
    assert det["image_width"] == image_bgr_dark.shape[1] and det["image_height"] == image_bgr_dark.shape[0]
...
def test_infer_uses_autocast_when_cuda(monkeypatch, image_bgr_dark):
    ...
    used = {"used": False}
    monkeypatch.setattr(yim, "autocast", lambda *a, **k: DummyCtx(flag=used), raising=True)
    monkeypatch.setattr(yim.torch, "inference_mode", lambda: DummyCtx(), raising=True)
...
def test_infer_fail_status_when_missing_items(monkeypatch, image_bgr_dark, cfg_cpu):
    ...
    class FakeDetectorMissing(FakeDetector):
        def process_detections(self, pred, processed_image, image, expected_items):
            rf, det, _ = super().process_detections(pred, processed_image, image, expected_items)
