...
- `開啟補訓資料與送出`：先建立獨立批次資料夾，再選圖、複核、補標與送訓。
- `補訓批次紀錄`：依`產品_工位_vX.Y.Z`查看不可變的選圖、複核結果與訓練參數。
- `補訓進度`：查看、續訓、安全停止或清除已終止的畫面紀錄。

同一產品／工位的批次資料夾不可重複。系統預設增加修訂版號；若需升次版或
主版，必須在開始選圖前建立正確名稱，不得用新內容覆寫已存在的資料夾。

### 5.2 檢測評估版本

此工作區統一管理`模型與顏色版本`、`候選組合`、`組合驗收`與`上線與回退`：

- 元件可包含 YOLO、Anomalib、完整顏色基準與顏色方案；
- 候選組合可自由選擇 Pipeline Template 允許的元件，不直接影響產線；
- 組合驗收使用獨立且已人工確認的驗收照片；選取 DRAFT 後可直接鎖定該版
  模型、config、完整顏色基準與逐色修訂，只驗收一個精確組合；
- 驗收完成會對原版本新增不可變驗收證明並顯示為 TESTED，不另建新版本；
