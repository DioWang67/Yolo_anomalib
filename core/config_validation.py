...
def validate_model_cfg(cfg: Dict[str, Any], product: str, area: str, selected_backend: str | None = None) -> None:
    ...
    if enable_yolo:
        ...
        if not weights:
            raise ValueError("YOLO 'weights' is required when enable_yolo=True")
    ...
    if cfg.get("enable_color_check", False):
        ...
        if not color_path:
            raise ValueError("enable_color_check=True but 'color_model_path' not set")
    ...
    if cfg.get("enable_anomalib", False):
        ...
        if not m or not m.get("ckpt_path"):
            raise ValueError(f"Anomalib ckpt_path missing for {product},{area}")
