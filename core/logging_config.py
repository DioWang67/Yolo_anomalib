    ...
    def log_detection(
        ...
        try:
            ...
            for det in items:
                ...
                cx = det.get("cx")
                cy = det.get("cy")
                offset = det.get("position_offset") or {}
                dx = offset.get("dx") if isinstance(offset, dict) else None
                ...
                position_map[cls].append(entry)

            if position_map:
                for cls, entries in position_map.items():
                    total = len(entries)
                    for idx, entry in enumerate(entries, 1):
                        expected = entry.get("expected") or {}
                        ex_cx = expected.get("cx") if isinstance(expected, dict) else None
                        ex_cy = expected.get("cy") if isinstance(expected, dict) else None
                        dx = entry.get("dx")
                        dy = entry.get("dy")
                        distance = entry.get("distance")
                        self.logger.info(
                            "Class %s #%d/%d: center=(%.1f, %.1f), expected=(%s, %s), "
                            "offset=(dx=%s, dy=%s), dist=%spx, status=%s",
                            cls,
                            idx,
                            total,
                            entry["cx"],
                            entry["cy"],
                            f"{ex_cx:.1f}" if isinstance(ex_cx, (int, float)) else "N/A",
                            f"{ex_cy:.1f}" if isinstance(ex_cy, (int, float)) else "N/A",
                            f"{dx:.1f}" if isinstance(dx, (int, float)) else "N/A",
                            f"{dy:.1f}" if isinstance(dy, (int, float)) else "N/A",
