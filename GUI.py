...
class DetectionSystemGUI(QMainWindow):
    ...
    def create_control_panel(self):
        ...
        try:
            self.product_combo.currentTextChanged.connect(
                lambda _=None: self.update_start_enabled()
            )
        except Exception:
            pass
        ...
        try:
            self.inference_combo.currentTextChanged.connect(
                lambda _=None: self.update_start_enabled()
            )
        except Exception:
            pass
        ...
        self.pick_image_btn.setStyleSheet(
        ...
        try:
            ...
            if self.available_products:
                ...
                try:
                    ...
                    if last_infer and last_infer in [
    ...
    def _load_image_with_retry(
        self, viewer, path, attempts=5, delay_ms=200, on_fail=None
    ):
    ...
    def start_detection(self):
        ...
        config_path = os.path.join(
            self._models_base, product, area, inference_type, "config.yaml"
        )
        if not os.path.exists(config_path):
            QMessageBox.critical(
                self, "模型不存在", f"找不到對應的模型設定檔:\n{config_path}"
            )
            return
        ...
        if not self.detection_system:
            QMessageBox.critical(
                self, "偵測系統", "偵測系統尚未成功初始化，請重新載入設定或重啟應用程式"
            )
            self.init_system()
            if not self.detection_system:
        ...
        try:
            ...
            pass
        if not use_cam:
            if getattr(self, "selected_image_path", None) and os.path.exists(
        ...
        self.worker = DetectionWorker(
            self.detection_system, product, area, inference_type, frame=frame
        )
    ...
    def closeEvent(self, event):
        ...
        try:
            ...
            self._settings.setValue(
                "last_product", self.product_combo.currentText())
            self._settings.setValue("last_area", self.area_combo.currentText())
            self._settings.setValue(
                "last_infer", self.inference_combo.currentText())
        except Exception:
            pass
