...
# < \~chinese 未定义的点云格式                  \~english Undefined point cloud format
...
# < \~chinese 内部用于AccessMode循环检测         \~english used internally for AccessMode cycle detection
AM_CycleDetect = 6
# < \~chinese 对象未被初始化                     \~english Object is not yet initialized
AM_Undefined = 5
...
# < \~chinese 等待下一帧图像                     \~english Wait for the next image
MV_GrabStrategy_UpcomingImage = 3
# < \~chinese 获取列表中最新的图像               \~english Gets the latest image in the list
MV_GrabStrategy_LatestImages = 2
# < \~chinese 获取列表中最新的一帧图像（同时清除列表中的其余图像）         \~english Gets the most recent image in the list (while clearing the rest of the images in the list)
MV_GrabStrategy_LatestImagesOnly = 1
# < \~chinese OBJ点云格式                       \~english The point cloud format named OBJ
MV_PointCloudFile_OBJ = 3
# < \~chinese CSV点云格式                       \~english The point cloud format named CSV
MV_PointCloudFile_CSV = 2
# < \~chinese PLY点云格式                       \~english The point cloud format named PLY
...
# < \~chinese 自动连续曝光模式                 \~english Exposure auto mode continuous
...
# < \~chinese 从旧到新一帧一帧的获取图像        \~english Frame by frame from old to new
MV_GrabStrategy_OneByOne = 0
MV_FormatType_AVI = (
    1  # < \~chinese AVI视频格式                     \~english AVI format type
)
# < \~chinese gamma选择项User                 \~english This enumeration selects the type of gamma to apply
...
# < \~chinese gamma选择项SRGB                 \~english This enumeration selects the type of gamma to apply
MV_GAMMA_SELECTOR_SRGB = 2
IFT_IPort = 11  # < \~chinese IPort接口类型                   \~english IPort interface
# < \~chinese 白平衡自动连续                   \~english Balance white auto continuous
...
# < \~chinese 表示组播模式，但本实例不接收图像数据         \~english Multicast without receive data
MV_GIGE_TRANSTYPE_MULTICAST_WITHOUT_RECV = 65537
IFT_IBase = (
    1  # < \~chinese IBase接口类型                             \~english IBase interface
)
# < \~chinese 表示设置了单播，但本实例不接收图像数据       \~english Unicast without receive data
MV_GIGE_TRANSTYPE_UNICAST_WITHOUT_RECV = 65536
# < \~chinese 单次自动白平衡                             \~english Balance white auto once
MV_BALANCEWHITE_AUTO_ONCE = 2
# < \~chinese 表示局域网内广播，暂不支持                  \~english Limited broadcast mode,not support
MV_GIGE_TRANSTYPE_LIMITEDBROADCAST = 2
MV_GIGE_TRANSTYPE_MULTICAST = (
    1  # < \~chinese 表示组播                                  \~english Multicast mode
)
# < \~chinese 表示单播(默认)                            \~english Unicast mode(default)
MV_GIGE_TRANSTYPE_UNICAST = 0
# < \~chinese 表示从相机获取，暂不支持                   \~english Transtype from camera,not support
MV_GIGE_TRANSTYPE_CAMERADEFINED = 4
# < \~chinese 表示子网内广播，暂不支持                   \~english Subnet broadcast mode,not support
MV_GIGE_TRANSTYPE_SUBNETBROADCAST = 3
MV_EXPOSURE_MODE_TRIGGER_WIDTH = (
    1  # < \~chinese 曝光模式触发宽                            \~english Trigger width
)
# < \~chinese 表示用户自定义应用端接收图像数据Port号      \~english User Defined Receive Data Port
MV_GIGE_TRANSTYPE_UNICAST_DEFINED_PORT = 5
# < \~chinese 触发源变频器                              \~english Trigger source frequency converter
MV_TRIGGER_SOURCE_FrequencyConverter = 8
# < \~chinese 触发源计数器                              \~english Trigger source conuter
MV_TRIGGER_SOURCE_COUNTER0 = 4
# < \~chinese LINE3 触发源                             \~english Trigger source line3
MV_TRIGGER_SOURCE_LINE3 = 3
# < \~chinese LINE2 触发源                             \~english Trigger source line2
MV_TRIGGER_SOURCE_LINE2 = 2
# < \~chinese LINE1 触发源                             \~english Trigger source line1
MV_TRIGGER_SOURCE_LINE1 = 1
# < \~chinese LINE0 触发源                             \~english Trigger source line0
...
# < \~chinese 按当前IP地址排序（升序）         \~english Sorting by current IP（Ascending）
SortMethod_CurrentIP_ASC = 2
# < \~chinese 按当前IP地址排序（降序）         \~english Sorting by current IP（Descending）
...
# < \~chinese sRGB to linear RGB(仅色彩插值时支持，色彩校正时无效) \~english sRGB to linear RGB
MV_CC_GAMMA_TYPE_SRGB2LRGB = 4
# < \~chinese 异常的图像，该帧被丢弃            \~english abnormal image,the frame is discarded
MV_CC_STREAM_EXCEPTION_ABNORMAL_IMAGE = 0x4001
# < \~chinese 缓存列表溢出，清除最旧的一帧       \~english Cache list overflow, clear the oldest frame
MV_CC_STREAM_EXCEPTION_LIST_OVERFLOW = 0x4002
# < \~chinese 缓存列表为空，该帧被丢弃           \~english Cache list is empty,the frame is discarded
MV_CC_STREAM_EXCEPTION_LIST_EMPTY = 0x4003
MV_CC_STREAM_EXCEPTION_RECONNECTION = (
    0x4004  # < \~chinese 断流恢复                         \~english reconnect
)
# < \~chinese 断流,恢复失败,取流被中止    \~english reconnect fail, stream is  terminated
...
# < \~chinese 源图像按行拆分成多张图像         \~english Source image split into multiple images by line
MV_SPLIT_BY_LINE = 1
...
_MV_GIGE_DEVICE_INFO_._fields_ = [
    ...
    # < \~chinese 型号名称           \~english Mode name
...
_MV_FRAME_OUT_INFO_EX_._fields_ = [
    ...
    # < \~chinese 帧计数             \~english Frame counter
...
_MVCC_STRINGVALUE_T._fields_ = [
    # < \~chinese 当前值         \~english Current Value
...
_MV_CC_FRAME_SPEC_INFO_._fields_ = [
    # < \~chinese 设备水印时标      \~english Device frame-specific time scale
    # < \~chinese 秒数                  \~english The Seconds
    ("nSecondCount", c_uint),
    (
        "nCycleCount",
        c_uint,
